...
- [x] **WU9-Rectangle — Characterize native Rectangle during parametric migration**
  - Authorization: user explicitly approved focused tests while keeping the current Rectangle tool native; the broader goal remains a parametric project. This is a transitional compatibility contract, not a decision that native Rectangle is the final parametric representation.
  - Scope: focused browser characterization of two-click native creation, provisional preview, Escape/tool-change cancellation, commit, one-step undo/redo and persisted reload without implicit sketch conversion. Existing editor-core/renderer contracts remain unchanged.
  - Out of scope: four-edge sketch Rectangle operation, implicit conversion, new solver/constraint semantics, production behavior changes, Circle/Arc/Spline/Move/Trim, WU12.
  - Route: bounded E2E test writer in `tests/e2e/app.smoke.spec.ts` plus this ledger, then independent verifier. Focused runner: `corepack pnpm exec playwright test tests/e2e/app.smoke.spec.ts -g 'Rectangle lifecycle'`; root lint/typecheck and diff check.
  - Status: complete; independently verified. Work-unit commit: `0220847` (`test(web): characterize native Rectangle lifecycle`).
  - Writer evidence: `tests/e2e/app.smoke.spec.ts` adds `Rectangle lifecycle keeps its draft provisional and persists one native rectangle`. The browser test measures the page/canvas intersection for click coordinates; records the initial document revision and element IDs; verifies after first-click preview, Escape cancellation, and tool-change cancellation that both remain unchanged and there are no native rect or sketch elements; reloads after cancellation and verifies the same empty document state before proceeding. It then verifies two-click completion creates exactly one native `rect[data-element-id]`, no sketch group, one revision change, undo removal, redo with the same ID, and native representation after reload. This characterizes current native rectangle persistence only; it does not claim a final parametric model or four-edge operation. No production changes. Focused `corepack pnpm exec playwright test tests/e2e/app.smoke.spec.ts -g 'Rectangle lifecycle'` passed (2 including setup); `corepack pnpm lint`, `corepack pnpm typecheck`, and `git diff --check` passed (Git emitted a non-failing LF-to-CRLF warning for the ledger). Independent GPT-6 Luna verification reviewed both cancellation revision/ID and reload assertions, native-only commit and same-ID undo/redo/reload; focused Playwright (2 passed incl. setup), root lint, typecheck and `git diff --check` passed. Broader Rectangle semantics were not tested. Commit pending.
...
- [x] **WU11-B — Sketch-edge construction-role inspector UX**
  ...
  - Status: complete; independently verified. Work-unit commit: `71e300a` (`feat(web): expose sketch roles in inspector`).
  - Writer evidence: added public Playwright coverage for click-created multi-edge sketch selection through Forma, targeted edge role isolation (the sibling remains normal), role styling on the selected edge, undo/redo, whole-sketch selection without a Forma edge key, and native role regression. Initial environment/setup RED: `corepack pnpm exec playwright test tests/e2e/project-workflow.spec.ts -g 'changes one sketch edge role from Forma inspector without changing its sibling'` timed out because `/modelo` did not enter an editor in this direct workflow spec; corrected to create project/piece via public UI. Behavior RED: same focused test then failed because the sketch-edge inspector role control was absent. GREEN: the initial edge/native filtered run passed (3 behavior tests plus setup). Whole-sketch computed browser style now proves inherited `stroke-dasharray` is `6px, 4px` on both child lines, with computed `fill: none`; undo restores `none` dash arrays and redo restores construction styling. Final focused run `corepack pnpm exec playwright test tests/e2e/project-workflow.spec.ts -g 'changes the whole selected sketch role when no Forma edge is selected|changes one sketch edge role from Forma inspector without changing its sibling|changes one native line role from the inspector with history and persisted reload'` passed (3 behavior tests plus setup). Multi-edge hide behavior remains an explicit coverage limit: there is no stable existing interaction/assertion for the ambiguous inspector state. No renderer change was warranted; no pointer routing, schema, solver, persistence, or selection semantics changed.
  - Writer validation: `corepack pnpm lint`, `corepack pnpm typecheck`, and `git diff --check` passed. `git diff --check` emitted only Git's non-failing LF-to-CRLF warnings. Independent GPT-6 Luna verification passed the focused browser filter (4 including setup), root lint, typecheck, unit tests (760/760), E2E (80 passed, 1 skipped), build and `git diff --check`. SVG construction dashes on a whole sketch are inherited by child lines; the earlier renderer-blocker hypothesis was disproven by computed browser style. Build chunk-size and Git line-ending warnings were non-blocking. Commit pending.

- [x] **WU11-C1 — Reproducible package performance baseline**
  - Authorization: WU11 performance evidence after focused UX slices. Measure fixed-size editor gesture/solver and SVG generation operations separately with warmup, samples, scale/iteration diagnostics and p50/p95/p99; record runtime/hardware/revision and honest API boundaries.
  - Out of scope: claiming package timings are browser pointermove latency or isolated preview/commit solve when not separately observable; workers/index/cache, universal thresholds, product optimization, WU12.
  - Likely surfaces: dedicated optional benchmark harness outside normal unit discovery and audit result section. Verify actual runner support before writing; commit harness and machine-specific observations as one bounded unit.
  - Status: complete; independently verified. Work-unit commit: `772cf85` (`perf(audit): capture sketch package baselines`).
  - Writer evidence: fixed deterministic public API workloads execute through Vitest's existing `vite-node` runner, which executes each module once; adaptive `vitest bench` was rejected for the custom collection because it repeats callbacks and chooses sample counts. Exact commands: `corepack pnpm exec vite-node --config vitest.config.ts packages/constraints/src/solver.bench.ts` and `corepack pnpm exec vite-node --config vitest.config.ts packages/editor-core/src/performance-baseline.bench.ts`. Each case used 20 warmups and 100 timed samples with nearest-rank p50/p95/p99. The editor preview timing encloses `createEditor → beginGesture → moveElements → previewGestureFromBase`; commit encloses the same sequence plus `commitGesture`, not isolated preview/commit solve. Solver scale 10/50/100 converged in 2 iterations, with 0 diagnostics and 0 non-converged components. SVG sizes: 1,868/8,988/17,908 UTF-8 bytes at 10/50/100 lines. Timings, environment, and limits: `docs/audits/parametric-sketch-performance-baseline.md`. Captured benchmark base revision: `1eb9559dd8cb740a8bddddf5f892d583c457a02d`; OS Windows 10 build 26200 x64, Node v24.14.0, AMD Ryzen 7 5700G with Radeon Graphics. Benchmark/audit edits were uncommitted during collection. No product source changed. Independent GPT-6 Luna verifier reran both harnesses and confirmed sample protocol, workload diagnostics, SVG byte counts and honest timing labels; root lint, typecheck, unit tests (760/760), and `git diff --check` passed. Rerun percentiles differed as expected on the same machine. Browser interaction remains WU11-C2; commit pending.

- [x] **WU11-C2 — Browser pointermove/interaction proxy baseline**
  - Authorization: WU11 pointermove evidence, separate from package microbenchmarks. Use a reproducible browser scenario and label event-to-frame timings as an end-to-end proxy, not isolated solve/render attribution. Record fixed viewport, workload, environment, samples and p50/p95/p99.
  - Out of scope: production instrumentation, perf optimization, pretending a proxy isolates kernel phases, unrelated UX workflow changes, WU12.
  - Status: complete; independently verified. Work-unit commit: `d9f6a00` (`perf(audit): capture browser pointer frame baseline`).
  - Evidence: dedicated `tests/e2e/editor.pointermove-frame.spec.ts` owns project/piece creation and native-rectangle workload through public UI; 10/25/50 rectangles are verified by SVG DOM counts. Each workload captures 120 serial actual hover `pointermove`-to-next-RAF samples after 20 warmups at 1440×1000, DPR 1, Chromium 140.0.7339.16 on Windows 10 x64; IDs and document revision remain unchanged. Observer uses bounded `arm`/`take`/`dispose`, timeout, and `try/finally` cleanup; validates finite, nonnegative timings before nearest-rank calculation. Exact command `corepack pnpm exec playwright test tests/e2e/editor.pointermove-frame.spec.ts --project=workflow-chromium --workers=1 --retries=0` passed 2/2 including setup in the corrected-clock writer run (20.0s; measured workflow 16.4s). Writer p50/p95/p99 ms: 10 = 10.900/12.800/13.500; 25 = 10.500/14.700/71.600; 50 = 9.700/20.200/34.900. Independent verifier reran the corrected-clock proxy (2 passed in 20.3s): 10 = 10.500/12.400/13.300; 25 = 10.600/14.600/15.400; 50 = 9.900/19.500/40.000. Earlier exploratory runs used the RAF timestamp and are excluded as non-comparable; no percentile stability is claimed. Full method and proxy limitations are in `docs/audits/parametric-sketch-performance-baseline.md`; architecture summary was updated. The default root E2E suite includes this additional ~16s descriptive proxy, with no CI threshold; no config change. Independent GPT-6 Luna verification passed root lint, typecheck, unit tests (760/760), E2E (81 passed, 1 skipped), build and `git diff --check`; build chunk-size warning was non-blocking. No product source changed.
