...
export function extractTextGlyphOutlines(text: TextElement, resolveFont: FontSourceResolver): readonly GlyphOutlineData[] {
  ...
  const totalAdvance = glyphs.reduce((sum, glyph, index) => sum + (kerning[index] ?? 0) + glyph.advanceWidth, 0) * text.fontSize / font.unitsPerEm;
  ...
  let advanceX = 0;
  const scale = text.fontSize / font.unitsPerEm;
  const rotate = (point: PointMm): PointMm => {
    const origin = { x: text.position.x + (advanceX * scale) + alignmentOffset, y: text.position.y + font.ascender * scale };
    const local = { x: point.x * scale + origin.x, y: origin.y - point.y * scale };
    const center = { x: text.position.x, y: text.position.y };
    const dx = local.x - center.x; const dy = local.y - center.y;
    return { x: center.x + dx * Math.cos(text.rotation) - dy * Math.sin(text.rotation), y: center.y + dx * Math.sin(text.rotation) + dy * Math.cos(text.rotation) };
  };
  ...
  for (const [index, glyph] of glyphs.entries()) {
    ...
    const finish = () => { if (nodes.length >= 2 && start && previous) { if (previous !== start) segments = [...segments, { type: "line", startNodeId: previous, endNodeId: start }]; contours.push({ nodes, segments }); } nodes = []; segments = []; start = undefined; previous = undefined; nodeCount = 0; };
    for (const command of commands) {
      if (command.type === "M") { finish(); const node = { id: `glyph-${index}-node-${nodeCount++}`, anchor: rotate({ x: command.x, y: command.y }), join: "corner" as const }; nodes = [node]; start = node.id; previous = node.id; }
      else if ((command.type === "L" || command.type === "C") && previous) {
        const end = { id: `glyph-${index}-node-${nodeCount++}`, anchor: rotate({ x: command.x, y: command.y }), join: "corner" as const };
        segments = [...segments, command.type === "C" ? { type: "cubicBezier" as const, startNodeId: previous, endNodeId: end.id, control1: rotate({ x: command.x1, y: command.y1 }), control2: rotate({ x: command.x2, y: command.y2 }) } : { type: "line" as const, startNodeId: previous, endNodeId: end.id }];
        nodes = [...nodes, end]; previous = end.id;
      } else if (command.type === "Q" && previous) {
        const end = { id: `glyph-${index}-node-${nodeCount++}`, anchor: rotate({ x: command.x, y: command.y }), join: "corner" as const };
        const start = nodes.at(-1)!.anchor;
        const quadratic = rotate({ x: command.x1, y: command.y1 });
