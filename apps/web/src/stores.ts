...
export const useViewportStore = create<{ zoom: number; panMm: PointMm; setZoom: (zoom: number) => void; setPanMm: (panMm: PointMm) => void }>((set) => ({ zoom: 3, panMm: { x: 20, y: 20 }, setZoom: (zoom) => set({ zoom: Math.max(0.5, Math.min(8, zoom)) }), setPanMm: (panMm) => set({ panMm }) }));
export const usePersistenceStore = create<{ state: "saved" | "saving" | "offline" | "recovered"; message: string; set: (state: "saved" | "saving" | "offline" | "recovered", message?: string) => void }>((set) => ({ state: "saved", message: "Documento local listo", set: (state, message = "") => set({ state, message }) }));
