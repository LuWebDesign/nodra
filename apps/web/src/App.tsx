     ...
     if (tool === "cut") {
        ...
        if (hit && element?.type === "line") {
          ...
          if (next !== editorRef.current) {
            const crossing = cuttableSegments(element).filter((candidate) => candidate.segmentIndex === hit.segmentIndex).flatMap((source) => next.document.elements.filter((candidate) => candidate.type !== "dimension" && candidate.id !== hit.elementId).flatMap((other) => cuttableSegments(other).map((target) => ({ target, intersection: lineSegmentIntersection(source.start, source.end, target.start, target.end) } )).filter((candidate) => candidate.intersection).map((candidate) => ({ target: candidate.target, point: candidate.intersection!.point }))));
            for (const crossingTarget of crossing) {
              const target = crossingTarget.target; const targetPoint = crossingTarget.point;
              const targetElement = next.document.elements.find((candidate) => candidate.id === target.elementId);
              next = targetElement?.type === "sketch" ? dispatch(next, cutSketchEdge(target.elementId, target.segmentIndex, targetPoint)) : targetElement?.type === "line" ? dispatch(next, cutLineAtPoint(target.elementId, targetPoint)) : targetElement?.type === "rectangle" || targetElement?.type === "ellipse" || targetElement?.type === "path" ? dispatch(next, cutPathSegment(target.elementId, target.segmentIndex, targetPoint)) : next;
            }
            setEditorState(select(next, next.document.elements.some((candidate) => candidate.id === hit.elementId) ? [hit.elementId] : []));
          }
