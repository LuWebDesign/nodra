...
import { positionalConnectionPair, type PositionalNodeReference } from "./positionalSelection.js";
...
const mark = globalThis.document.createElementNS("http://www.w3.org/2000/svg", "rect");
  ...
  useEffect(() => {
    ...
    const selectedCircularSource = selectedPositionalPair?.source;
    const selectedPositionalTarget = selectedPositionalPair?.target;
    ...
    const globalResiduals = useMemo(() => new Map(constraintResidualsForDocument(document).map((residual) => [residual.constraintId, residual])), [document]);
        ...
        const confirmConstraintDraft = () => { if (!constraintDraft) return; setEditorState(commitGesture(editorRef.current)); setConstraintDraft(undefined); };
         ...
         const addNativePositionRelation = () => {
           ...
           const next = dispatch(editorRef.current, addPositionalConnection({ elementId: selectedCircularSource.elementId, node: selectedCircularSource.address }, { elementId: selectedPositionalTarget.elementId, node: selectedPositionalTarget.address }));
