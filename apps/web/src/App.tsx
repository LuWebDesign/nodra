...
const mark = globalThis.document.createElementNS("http://www.w3.org/2000/svg", "rect");
  ...
  const marqueeStyle = marquee ? (() => {
   ...
   const pendingDimensionOverlay = (tool === "dimension" || tool === "radius") && dimensionDraft && cursorPoint ? (() => {
     ...
     const cutSegmentHoverOverlay = tool === "cut" && cutSegmentHover ? (() => {
       const hoveredElement = document.elements.find((element) => element.id === cutSegmentHover.elementId);
       const hoveredSegment = hoveredElement?.type === "path" ? hoveredElement.segments[cutSegmentHover.segmentIndex] : undefined;
       const hoveredStart = hoveredElement?.type === "path" ? hoveredElement.nodes.find((node) => node.id === hoveredSegment?.startNodeId)?.anchor : undefined;
       const hoveredEnd = hoveredElement?.type === "path" ? hoveredElement.nodes.find((node) => node.id === hoveredSegment?.endNodeId)?.anchor : undefined;
       const cubicPath = hoveredSegment?.type === "cubicBezier" && hoveredStart && hoveredEnd ? `M ${hoveredStart.x} ${hoveredStart.y} C ${hoveredSegment.control1.x} ${hoveredSegment.control1.y}, ${hoveredSegment.control2.x} ${hoveredSegment.control2.y}, ${hoveredEnd.x} ${hoveredEnd.y}` : undefined;
       return <svg className="cut-segment-hover-overlay" viewBox={`0 0 ${document.page.width} ${document.page.height}`} style={{ left: pageStyle.left + 1, top: pageStyle.top + 1, width: document.page.width * zoom, height: document.page.height * zoom, right: "auto", bottom: "auto" }} aria-label="Segmento de corte bajo el puntero">{cubicPath ? <path d={cubicPath} /> : cutSegmentHover.points ? <polyline points={cutSegmentHover.points.map((point) => `${point.x},${point.y}`).join(" ")} /> : <line x1={cutSegmentHover.start.x} y1={cutSegmentHover.start.y} x2={cutSegmentHover.end.x} y2={cutSegmentHover.end.y} />}</svg>;
     })() : undefined;
