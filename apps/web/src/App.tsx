...
export function App() {
  ...
  const setSelectedGeometryRole = (role: "normal" | "construction") => {
    if (selection.length !== 1 || !selectedElement || !isNativeGeometricElement(selectedElement)) return;
    const next = dispatch(editorRef.current, setGeometryRole({ kind: "element", elementId: selectedElement.id }, role));
