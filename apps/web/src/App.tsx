...
const shouldDriveCircleDimension = (dimension: DimensionElement, elements: readonly Element[]): boolean => (dimension.kind === "radius" || dimension.kind === "diameter") && elements.find((element) => element.id === dimension.references[0].elementId)?.type === "circle";
const shouldCreateSketchDrivingConstraint = (dimension: DimensionElement): boolean => {
  const [first, second] = dimension.references;
  return first.elementId === second.elementId && (dimension.kind === "aligned" || dimension.kind === "horizontal" || dimension.kind === "vertical" || dimension.kind === "angular");
};
...
const mark = globalThis.document.createElementNS("http://www.w3.org/2000/svg", "rect");
      ...
      if (tool === "forma" && rawFormaNodeHit) {
        ...
        if ((tool === "dimension" || tool === "radius")) {
           if (dimensionDraft?.phase === "first" && dimensionDraft.first.kind === "circle") {
             ...
             const next = shouldDriveCircleDimension(dimension, created.document.elements) ? dispatch(created, setDimensionDriving(dimension.id, true)) : created;
           ...
           if (dimensionDraft?.phase === "placement") {
            ...
            const created = dispatch(editorRef.current, createElement(dimension));
            if (created === editorRef.current) return;
            const next = shouldDriveCircleDimension(dimension, created.document.elements) || shouldCreateSketchDrivingConstraint(dimension) ? dispatch(created, setDimensionDriving(dimension.id, true)) : created;
  ...
  const onCanvasPointerMove = (event: PointerEvent<HTMLDivElement>) => {
     ...
     if (tool === "cut" && !interaction.current) setCutSegmentHover(pickCutIntervalPreview(editorRef.current.document, pointAt(event), zoom));
