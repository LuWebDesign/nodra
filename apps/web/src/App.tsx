...
import { pathJoinGuidance, pathJoinOptions } from "./pathJoins.js";
    import { addPage, addPiece, configureInitialProject, dashboardProjects, deletePiece, nextPieceName, pieceDisplayLabel, piecePageId, projectDetail, projectDisplayName, projectTree, renamePage, renamePiece, renameProjectMetadata, selectPiecePage, type DashboardProjectSource } from "./projectDashboard.js";
...
const isNativeGeometricElement = (element: Element): boolean => element.type === "rectangle" || element.type === "circle" || element.type === "arc" || element.type === "ellipse" || element.type === "line" || element.type === "contour" || element.type === "path" || element.type === "spline" || element.type === "glyph";
...
const newDimension = (layer: string, first: NodeHit, second: NodeHit, placement: PointMm, mode: DimensionMode = "auto"): DimensionElement => { const midpoint = pointMidpoint(first.node.point, second.node.point); const rimKinds = ["edge-midpoint", "cardinal", "endpoint"]; const sameCircularElement = first.elementId === second.elementId && (first.node.kind === "center" && rimKinds.includes(second.node.kind) || second.node.kind === "center" && rimKinds.includes(first.node.kind)); const kind = sameCircularElement ? mode === "radius" ? "radius" : "diameter" : dimensionKindForPlacement(first.node.point, second.node.point, placement); const circularCenter = first.node.kind === "center" ? first.node.point : second.node.point; return { type: "dimension", id: id(), layerId: layerId(layer), kind, references: [{ kind: "node", elementId: first.elementId, nodeIndex: first.nodeIndex, ...(first.node.nodeId && first.node.kind !== "control" ? { nodeId: first.node.nodeId } : {}) }, { kind: "node", elementId: second.elementId, nodeIndex: second.nodeIndex, ...(second.node.nodeId && second.node.kind !== "control" ? { nodeId: second.node.nodeId } : {}) }], offset: kind === "diameter" || kind === "radius" ? { x: placement.x - (sameCircularElement ? circularCenter.x : midpoint.x), y: placement.y - (sameCircularElement ? circularCenter.y : midpoint.y) } : kind === "aligned" ? dimensionOffsetForAlignedPlacement(first.node.point, second.node.point, placement) : dimensionOffsetForPlacement(kind, midpoint, placement), precision: 2, units: "mm", rotation: 0, style: { stroke: "#2563eb", strokeWidth: 0.45 } }; };
...
const previewFragmentShape = (fragment: CurveFragment, key: string): ReactNode => {
  const curve = fragment.curve;
  if (curve.type === "line") return <line key={key} x1={curve.start.x} y1={curve.start.y} x2={curve.end.x} y2={curve.end.y} />;
  if (curve.type === "cubicBezier") return <path key={key} d={`M ${curve.p0.x} ${curve.p0.y} C ${curve.p1.x} ${curve.p1.y} ${curve.p2.x} ${curve.p2.y} ${curve.p3.x} ${curve.p3.y}`} />;
  if (curve.type === "circle" || curve.fullTurn) return <circle key={key} cx={curve.center.x} cy={curve.center.y} r={curve.radius} />;
  const direction = curve.direction === "clockwise" ? 1 : -1;
  const sweep = ((curve.endAngle - curve.startAngle) * direction % (Math.PI * 2) + Math.PI * 2) % (Math.PI * 2);
  const start = { x: curve.center.x + curve.radius * Math.cos(curve.startAngle), y: curve.center.y + curve.radius * Math.sin(curve.startAngle) };
  const end = { x: curve.center.x + curve.radius * Math.cos(curve.endAngle), y: curve.center.y + curve.radius * Math.sin(curve.endAngle) };
  return <path key={key} d={`M ${start.x} ${start.y} A ${curve.radius} ${curve.radius} 0 ${sweep > Math.PI ? 1 : 0} ${curve.direction === "clockwise" ? 1 : 0} ${end.x} ${end.y}`} />;
};
...
/** Derives a transient profile preview only from an explicit active selection scope. */
export const profilePreviewForSelection = (
...
export function App() {
  ...
  const [nativeProjectPath, setNativeProjectPath] = useState<string>();
      ...
      const [pendingSketchAcceptance, setPendingSketchAcceptance] = useState(false);
      const [newSketchName, setNewSketchName] = useState("");
      const [activePieceId, setActivePieceId] = useState<PieceId | undefined>(() => project.pieces[0]?.id);
      ...
      const [pieceFormProjectId, setPieceFormProjectId] = useState<string>();
       const [editingPieceId, setEditingPieceId] = useState<PieceId>();
      const [pieceName, setPieceName] = useState("");
      const [pieceMaterial, setPieceMaterial] = useState("");
      const [pieceThickness, setPieceThickness] = useState("");
      const [newProjectDraft, setNewProjectDraft] = useState<{ projectName: string }>();
      const [deleteProjectId, setDeleteProjectId] = useState<string>();
      const [deletePieceId, setDeletePieceId] = useState<PieceId>();
      const [renameDraft, setRenameDraft] = useState<string>();
          const [entityRename, setEntityRename] = useState<{ type: "piece" | "page"; id: string; value: string }>();
      const pendingNavigationRef = useRef<(() => void) | undefined>(undefined);
          const draftProjectRef = useRef<ProjectSnapshot | undefined>(undefined);
  ...
  const creationDraftRef = useRef<CreationDraft | undefined>(undefined);
      const releasePointerCapture = (pointerId: number) => {
        ...
        if (!target) return;
        for (const candidate of [target, ...Array.from(target.querySelectorAll<globalThis.Element>("*"))]) {
          if (candidate.hasPointerCapture(pointerId)) candidate.releasePointerCapture(pointerId);
        }
      };
  const viewportInteracted = useRef(false);
      const viewportByPage = useRef(new Map<string, { readonly zoom: number; readonly panMm: PointMm }>());
  ...
  const activePiece = activePieceId === undefined ? undefined : project.pieces.find((piece) => piece.id === resolveActivePieceId(project, activePieceId));
      ...
      useEffect(() => {
        collapsedPagesProjectRef.current = project.id;
        setCollapsedPages(loadCollapsedPages(project.id));
      }, [project.id]);

      useEffect(() => {
        if (collapsedPagesProjectRef.current === project.id) saveCollapsedPages(project.id, collapsedPages);
      }, [collapsedPages, project.id]);
  ...
  useLayoutEffect(() => {
    pageElement.current = canvasElement?.querySelector<HTMLDivElement>(".page") ?? null;
    return () => { pageElement.current = null; };
  }, [canvasElement, document.id, document.page.width, document.page.height, view]);
  ...
  useLayoutEffect(() => {
    ...
    if (!target) return;
    const key = `${project.id}:${project.activePageId}`;
        const saved = viewportByPage.current.get(key);
        if (saved) {
          setZoom(saved.zoom);
          setPanMm(saved.panMm);
          viewportInteracted.current = true;
    ...
    const center = () => {
      if (viewportInteracted.current || centeredViewport.current === key) return;
  ...
  }, [canvasElement, document.id, document.page.width, document.page.height, project.id, project.activePageId, setPanMm, setZoom, view]);
  ...
  const selectedSketchRoleEdges = selectedElement?.type === "sketch"
    ? selectedElement.edges.filter((edge) => selectedFormaEdgeKeys.includes(`${selectedElement.id}:e:${edge.id}`))
    : [];
  const selectedSketchRoleTarget = selectedElement?.type === "sketch" && selection.length === 1
    ? selectedSketchRoleEdges.length === 1 && selectedFormaEdgeKeys.length === 1
      ? { kind: "sketch-edge" as const, elementId: selectedElement.id, edgeId: selectedSketchRoleEdges[0]!.id }
      : selectedSketchRoleEdges.length === 0 && selectedFormaEdgeKeys.length === 0 && selectedFormaNodeKeys.length === 0
        ? { kind: "element" as const, elementId: selectedElement.id }
        : undefined
    : undefined;
  const setSelectedGeometryRole = (role: "normal" | "construction") => {
    if (selection.length !== 1 || !selectedElement) return;
    const target = isNativeGeometricElement(selectedElement)
      ? { kind: "element" as const, elementId: selectedElement.id }
      : selectedSketchRoleTarget;
    if (!target) return;
    const next = dispatch(editorRef.current, setGeometryRole(target, role));
    if (next !== editorRef.current) setEditorState(next);
  };
  const selectedBounds = selectedElements.length ? boundsOfElements(selectedElements) : undefined;
  const selectedRadialDimensionTarget = selectedElement?.type === "dimension" && (selectedElement.kind === "radius" || selectedElement.kind === "diameter")
    ? document.elements.find((element) => element.id === selectedElement.references[0].elementId)
    : undefined;
  ...
  }, [canvasElement, document, mode, panMm, selectedElement, tool, transformMode, zoom, view]);
  ...
  useEffect(() => {
    ...
    if (transformMode === "resize" && tool !== "forma") for (const element of selectedElements.filter((candidate) => candidate.type !== "spline" && candidate.type !== "text" && candidate.type !== "arc")) for (const [nodeIndex, node] of realGeometryNodes(element).entries()) {
...
const mark = globalThis.document.createElementNS("http://www.w3.org/2000/svg", "rect");
  ...
  }, [canvasElement, cursorPoint, document, panMm, selectedElements, snapGuide, tool, transformMode, zoom, view]);
  ...
  useEffect(() => {
        if (persistenceReady) saveLastAppLocation({ view, mode });
    if (persistenceReady && activeProjectMetadata.id === project.id && sketchSessionRef.current.status === "idle") saveLastOpenedProject({ projectId: project.id, ...(activePieceId === undefined ? {} : { pieceId: activePieceId }), pageId: project.activePageId, view, mode });
  ...
  const setEditorState = (next: typeof editor) => {
     next = { ...next, document: { ...next.document, elements: next.document.elements.map((element) => activePiece ? element.pieceId === undefined ? { ...element, pieceId: activePiece.id } : element : element) } };
  ...
  const enterSketchSession = (sketchId: ElementId, sourceEditor = editorRef.current) => {
      ...
      const acceptSketchSession = () => {
        ...
        if (draftProjectRef.current) {
          setNewSketchName(activePiece?.name || nextPieceName(project));
          setPendingSketchAcceptance(true);
          return;
        }
        setEditor(editorRef.current, { syncProject: true });
      ...
      const confirmSketchAcceptance = () => {
        const baseline = draftProjectRef.current;
        const current = sketchSessionRef.current;
        if (!baseline || current.status !== "active") return;
        try {
          const activePage = baseline.activePageId;
          const pendingPiece = creationPending ? project.pieces.find((piece) => piece.id === creationPending) : undefined;
          const activePiece = baseline.pieces.find((piece) => piece.id === activePieceId && piecePageId(baseline, piece) === activePage);
          const committedBase = pendingPiece ? addPiece(baseline, { name: newSketchName || pendingPiece.name || nextPieceName(baseline, activePage) }) : activePiece ? { ...baseline, activePieceId: activePiece.id } : addPiece(baseline, { name: newSketchName || nextPieceName(baseline, activePage) });
          const committed = projectWithSketchAssociation(committedBase, editorRef.current.document, committedBase.activePieceId, current.sketchId);
          draftProjectRef.current = undefined;
          setPendingSketchAcceptance(false);
          setCreationPending(undefined);
          setProject(committed);
          setActivePieceId(committed.activePieceId ?? committed.pieces[0]?.id);
          setEditor(editorRef.current, { syncProject: true });
          setSketchSession((state) => reduceSketchSession(state, { type: "accept" }));
          setEditModeElementIds([]); setEditorState(clearSelection(editorRef.current)); setTool("select");
        } catch (error) { persist.set("failed", error instanceof Error ? error.message : "No se pudo guardar el croquis"); }
      };
      ...
      const requestCancelSketchSession = () => {
        const current = sketchSessionRef.current;
        if (current.status !== "active") return;
        if (!hasSketchSessionChanges(current)) { setSketchSession(reduceSketchSession(current, { type: "cancel-session" })); if (draftProjectRef.current) { const draftProject = draftProjectRef.current; draftProjectRef.current = undefined; setProject(draftProject); setActivePieceId(draftProject.activePieceId ?? draftProject.pieces[0]?.id ?? activePieceId); setCreationPending(undefined); } confirmPendingNavigation(); return; }
            if (current.committed.length === 0) {
          restoreSketchSessionEntry(current);
          setSketchSession({ status: "confirming-cancel", sketchId: current.sketchId, document: current.document, editorHistory: current.editorHistory, entryDocument: current.entryDocument, entryEditorHistory: current.entryEditorHistory, entrySelection: current.entrySelection, committed: [] });
        } else {
          if (editorRef.current.gesture) {
      ...
      const startNewSketchOnPage = (source: DashboardProjectSource, pageIdValue: string) => guardedNavigation(() => {
        ...
        setActiveProjectMetadata(source.metadata); setProject(draft); setActivePieceId(draft.activePieceId ?? draft.pieces[0]?.id); setCreationPending(draft.activePieceId); setPendingDetailSketchId(undefined); setMode("design"); setView("editor"); setTool("line");
      });
      const openPersistedProject = async (metadata: ProjectMetadata) => {
        ...
        /* empty projects keep their persisted active page */
      ...
      const openNewPieceForm = (source: DashboardProjectSource) => {
       ...
       const createPersistedPiece = async (source: DashboardProjectSource) => {
        ...
        try {
          ...
          setActiveProjectMetadata(metadata); setProject(nextProject); setActivePieceId(nextProject.activePieceId ?? nextProject.pieces[0]?.id); setMode("design"); setView("editor"); setTool("select");
      ...
      const editPersistedPiece = async (source: DashboardProjectSource, pieceIdValue: PieceId) => {
         ...
         try {
           ...
           const nextProject = {
             ...renamedProject,
             pieces: renamedProject.pieces.map((piece) => {
                   if (piece.id !== pieceIdValue) return piece;
                   const updatedPiece = { ...piece };
                   if (material) updatedPiece.material = material;
                   else delete updatedPiece.material;
                   if (thicknessMm === undefined) delete updatedPiece.thicknessMm;
                   else updatedPiece.thicknessMm = thicknessMm;
                   return updatedPiece;
                 }),
           };
           const metadata = { ...source.metadata, updatedAt: Date.now() };
           ...
           setDashboardSources((current) => current.map((item) => item.metadata.id === source.metadata.id ? { metadata, project: nextProject } : item));
           saveProjectMirror(nextProject);
           setEditingPieceId(undefined); setPieceFormProjectId(undefined); setPieceName(""); setPieceMaterial(""); setPieceThickness("");
           persist.set("saved", "Pieza actualizada");
           openProjectInDesign({ metadata, project: nextProject }, pieceIdValue);
         } catch (error) { persist.set("failed", error instanceof Error ? error.message : "No se pudo editar la pieza"); }
       };
       const createPersistedPage = async () => {
            try {
              ...
              setActiveProjectMetadata(metadata); setProject(nextProject); setActivePieceId(nextProject.pieces.at(-1)!.id); saveProjectMirror(nextProject);
              persist.set("saved", "Página creada");
            } catch (error) { persist.set("failed", error instanceof Error ? error.message : "No se pudo crear la página"); }
          };
      ...
      const renamePersistedEntity = async (source: DashboardProjectSource, entity: { type: "piece" | "page"; id: string; value: string }) => {
            try {
              ...
              lastOfficialSave.current = { projectId: metadata.id, snapshot: JSON.stringify(nextProject) };
              setDashboardSources((current) => current.map((item) => item.metadata.id === metadata.id ? { metadata, project: nextProject } : item));
              if (activeProjectMetadata.id === metadata.id) { setActiveProjectMetadata(metadata); setProject(nextProject); }
              saveProjectMirror(nextProject); setEntityRename(undefined); persist.set("saved", `${entity.type === "piece" ? "Pieza" : "Página"} renombrada`);
        ...
        try {
          ...
          lastOfficialSave.current = { projectId: metadata.id, snapshot: JSON.stringify(nextProject) };
          setDashboardSources((current) => current.map((item) => item.metadata.id === metadata.id ? { metadata, project: nextProject } : item));
          if (activeProjectMetadata.id === metadata.id) { setActiveProjectMetadata(metadata); setProject(nextProject); setActivePieceId(nextProject.activePieceId ?? nextProject.pieces[0]?.id); }
          saveProjectMirror(nextProject);
      ...
      const createPersistedProject = async () => {
        if (!newProjectDraft) return;
          ...
          const projectId = `nodra-project-${crypto.randomUUID()}`;
              setDetailProjectId(projectId); setNewProjectDraft(undefined); setView("project");
  ...
  const saveBrowserProject = (snapshot: ProjectSnapshot, metadata: ProjectMetadata): Promise<void> => browserSaveQueue.current.enqueue(async () => {
    ...
    useEffect(() => {
      ...
      const metadata = { ...activeProjectMetadata, updatedAt: Date.now() };
      void saveBrowserProject(committedProject, metadata);
    }, [activeProjectMetadata, persistenceReady, project, savePolicy.mode]);
    ...
    if (tool === "pan") {
      ...
      if (tool === "arc") {
        ...
        if (!draft) {
          ...
          const nextDraft = { ...draft, points: [...draft.points, creationPoint], pointer: creationPoint, snaps: [...(draft.snaps ?? []), creationSnap] } as const;
          creationDraftRef.current = nextDraft;
          setCreationDraft(nextDraft);
        } else {
          const curve = arcThroughThreePoints(draft.points[0]!, draft.points[1]!, creationPoint);
          if (!curve) {
            // Keep the two confirmed endpoints so the user can correct only the bulge point.
            const invalidDraft = { ...draft, pointer: creationPoint };
            creationDraftRef.current = invalidDraft;
            setCreationDraft(invalidDraft);
            return;
          }
          const arc: ArcElement = { type: "arc", id: id(), layerId: layerId(editorRef.current.document.layers[0]?.id ?? "layer-1"), center: curve.center, radius: curve.radius, startAngle: curve.startAngle, endAngle: curve.endAngle, direction: curve.direction, style: defaultStyle };
          const next = dispatch(editorRef.current, createElement(arc, creationConnections(arc, (draft.snaps ?? []).slice(0, 2))));
          if (next !== editorRef.current) setEditorState(select(next, [arc.id]));
          creationDraftRef.current = undefined;
          setCreationDraft(undefined);
        }
        return;
      ...
      if (tool === "forma" && rawFormaNodeHit) {
        ...
        if ((tool === "dimension" || tool === "radius")) {
           if (dimensionDraft?.phase === "first" && dimensionDraft.first.kind === "circle") {
             ...
             const next = dimensionDrivingCapability(dimension, created.document.elements) === "driving" ? dispatch(created, setDimensionDriving(dimension.id, true)) : created;
          ...
          if (dimensionDraft.phase === "first") {
           ...
           if (dimensionDraft.first.kind === "line" && dimensionTarget.kind === "line" && dimensionDraft.first.hit.elementId === dimensionTarget.hit.elementId && (dimensionDraft.first.hit.edgeId !== undefined || dimensionTarget.hit.edgeId !== undefined ? dimensionDraft.first.hit.edgeId !== undefined && dimensionDraft.first.hit.edgeId === dimensionTarget.hit.edgeId : (dimensionDraft.first.hit.edgeIndex ?? 0) === (dimensionTarget.hit.edgeIndex ?? 0))) {
  ...
  const onCanvasPointerMove = (event: PointerEvent<HTMLDivElement>) => {
    ...
    if (!active.start || !active.tool || !isDrawingTool(active.tool) || active.tool === "arc" || !active.ids?.[0] || !active.startClient || !movementExceedsThreshold(active.startClient, { x: event.clientX, y: event.clientY })) return;
  ...
  useEffect(() => {
    ...
    const selectedSketchConstraintState = useMemo<ConstraintState>(() => {
      if (selectedElement?.type !== "sketch") return "underdefined";
      const priority: Record<ConstraintState, number> = { "fully-defined": 0, underdefined: 1, overdefined: 2, conflict: 3, invalid: 4 };
      const nodeKeys = new Set(selectedElement.nodes.map((node) => JSON.stringify([selectedElement.id, node.id])));
      return componentStates.filter((component) => component.nodeKeys.some((key) => nodeKeys.has(key))).reduce<ConstraintState>((current, component) => priority[component.state] > priority[current] ? component.state : current, "fully-defined");
    }, [componentStates, selectedElement]);
    const globalResiduals = useMemo(() => new Map(constraintResidualsForDocument(document).map((residual) => [residual.constraintId, residual])), [document]);
        const addRelationToSelectedSketch = (kind: SketchConstraint["kind"], value?: number) => {
          ...
          if (global && !supportsGlobalConstraintKind(kind)) return;
          const command: EditorCommand = global ? addSolvedDocumentConstraint(constraint) : addSketchConstraint(targetSketch.id, constraint);
  ...
  const marqueeStyle = marquee ? (() => {
   ...
   const pendingDimensionOverlay = (tool === "dimension" || tool === "radius") && dimensionDraft && cursorPoint ? (() => {
     ...
     const cursorNodeGuideOverlay = tool === "line" && documentCursorPoint ? (() => { const guides = cursorNodeGuides(document, documentCursorPoint, zoom, 5); return guides.length ? <svg className="node-guide-overlay" viewBox={`0 0 ${document.page.width} ${document.page.height}`} style={{ left: pageStyle.left + 1, top: pageStyle.top + 1, width: document.page.width * zoom, height: document.page.height * zoom, right: "auto", bottom: "auto" }} aria-label="Guía de nodo">{guides.map((guide, index) => <line key={`cursor-node-guide-${index}`} className="creation-guide creation-guide-node-alignment" x1={guide.source.x} y1={guide.source.y} x2={guide.target.x} y2={guide.target.y} />)}</svg> : undefined; })() : undefined;
      ...
      const arc = creationDraft.tool === "arc" && creationDraft.points.length >= 2 ? arcThroughThreePoints(creationDraft.points[0]!, creationDraft.points[1]!, pointer) : undefined;
  ...
  const rulerVertical = <div className="ruler-vertical" data-ruler-vertical="true" aria-hidden="true">
    {rulerYValues.map((value) => <span className="ruler-tick ruler-tick-vertical" key={`ruler-y-${value}`} style={{ top: (value - panMm.y) * zoom }}><i /><b>{value}</b></span>)}
       ...
       const selectedEditOverlay = tool === "forma" ? selectedElements.filter((element) => editModeElementIds.includes(element.id) && element.type !== "spline" && element.type !== "line" && element.type !== "text" && element.type !== "arc").map((element) => {
  ...
  const viewportKey = (pageIdValue: string) => `${project.id}:${pageIdValue}`;
      const rememberViewport = () => {
                viewportByPage.current.set(viewportKey(project.activePageId), { zoom, panMm });
      };
      const resetPageInteraction = (nextPageId?: string) => {
        rememberViewport();
            if (interaction.current) releasePointerCapture(interaction.current.pointerId);
    setTransformMode("resize");
    setCenterHover(undefined);
    setMarquee(undefined);
    interaction.current = undefined;
    const key = viewportKey(nextPageId ?? project.activePageId);
                const saved = viewportByPage.current.get(key);
                centeredViewport.current = saved ? key : undefined;
                viewportInteracted.current = Boolean(saved);
    setZoom(saved?.zoom ?? zoom);
    setPanMm(saved?.panMm ?? { x: 0, y: 0 });
  };
  const switchPage = (nextPageId: string) => {
    if (nextPageId === project.activePageId) return;
    guardedNavigation(() => {
    resetPageInteraction(nextPageId);
                const committedProject = sketchSessionRef.current.status === "idle" ? projectFromDocument(project, editorRef.current.document) : project;
                    setProject({ ...committedProject, activePageId: pageId(nextPageId) });
  });
  };
  const activatePiece = (pieceIdValue: PieceId) => {
    if (!activePiece || (pieceIdValue === activePieceId && piecePageId(project, activePiece) === project.activePageId)) return;
    guardedNavigation(() => {
        const piece = project.pieces.find((candidate) => candidate.id === pieceIdValue) ?? project.pieces[0]!;
        const committedProject = sketchSessionRef.current.status === "idle" ? projectFromDocument(project, editorRef.current.document) : project;
        const pageIdValue = piecePageId(committedProject, piece);
        setActivePieceId(piece.id);
        resetPageInteraction(pageIdValue);
        setProject({ ...selectPiecePage(committedProject, piece), activePieceId: piece.id, activePageId: pageIdValue });
      });
  };
  ...
  const mirrorButton = (axis: FlipAxis) => {
    const label = axis === "horizontal" ? "Espejar horizontal" : "Espejar vertical";
    ...
    return <button type="button" className={isActive ? "tool active selection-tool" : "tool selection-tool"} aria-label={label} aria-pressed={isActive} title={description} aria-description={description} onClick={() => {
      const current = editorRef.current;
      if (current.selection.length) setEditorState(dispatch(current, flipElements(current.selection, axis)));
    }}>
      <svg className="tool-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      ...
      <small>{label.replace("Espejar ", "")}</small><span className="tool-description" role="tooltip">{description}</span>
     </button>;
    };
  const rotateSelectionButton = () => <button type="button" className={transformMode === "rotate" ? "tool active selection-tool" : "tool selection-tool"} aria-label="Girar" aria-pressed={transformMode === "rotate"} title="Girar la selección." aria-description="Girar la selección." onClick={() => setTransformMode("rotate")}><svg className="tool-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M18 8a7 7 0 1 0 1 7" /><path d="m18 4 1 4-4-1" /><path d="M12 9v6M9 12h6" /></svg><small>Girar</small><span className="tool-description" role="tooltip">Girar la selección.</span></button>;
      const shapeOperations = () => <div className="shape-operation-group" role="group" aria-label="Operaciones de forma">
        ...
        <div className="shape-operation-buttons">
          {(() => {
            ...
            const isClosedShapeSelection = (element: Element) => element.type !== "line" && element.type !== "arc" && element.type !== "text" && element.type !== "dimension" && (element.type !== "path" && element.type !== "spline" || element.closed);
  ...
  const treeIcon = (type: "project" | "page" | "piece" | "sketch") => <span className={`tree-node-icon tree-node-icon-${type}`} aria-hidden="true"><svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.35" strokeLinecap="round" strokeLinejoin="round">{type === "project" ? <><path d="m2.5 5 5.5-2.5L13.5 5 8 7.5 2.5 5Z" /><path d="M2.5 5v6L8 13.5l5.5-2.5V5M8 7.5v6" /></> : type === "page" ? <><path d="M4 1.75h5l3 3v9.5H4z" /><path d="M9 1.75v3h3M6 8h4M6 10.5h4" /></> : type === "piece" ? <><path d="M3 12.5V5.75L8 3l5 2.75v6.75L8 15z" /><path d="M3 5.75 8 8.5l5-2.75M8 8.5V15" /></> : <><path d="m2.5 11 3-4 2.5 2 3.5-5 2 2.5" /><path d="M2.5 13.5h11" /></>}</svg></span>;
      const inlineRename = (source: DashboardProjectSource, type: "piece" | "page", id: string, label: string, activate?: () => void, className = "", icon?: ReactNode) => entityRename?.type === type && entityRename.id === id ? <span className="inline-rename" onClick={(event) => event.stopPropagation()}><input autoFocus aria-label={`Nuevo nombre de ${type === "piece" ? "la pieza" : "la página"}`} value={entityRename.value} onChange={(event) => setEntityRename((current) => current ? { ...current, value: event.target.value } : current)} onKeyDown={(event) => { if (event.key === "Escape") { event.preventDefault(); setEntityRename(undefined); } else if (event.key === "Enter") { event.preventDefault(); void renamePersistedEntity(source, entityRename); } }} onBlur={() => setEntityRename(undefined)} /><button type="button" onMouseDown={(event) => event.preventDefault()} onClick={() => void renamePersistedEntity(source, entityRename)}>Guardar</button><button type="button" onClick={() => setEntityRename(undefined)}>Cancelar</button></span> : <button type="button" className={className} onClick={activate} onDoubleClick={(event) => { event.preventDefault(); event.stopPropagation(); setEntityRename({ type, id, value: label }); }}>{icon}{label}</button>;
const renderProjectDetail = () => {
  const source = dashboardSources.find((candidate) => candidate.metadata.id === detailProjectId);
  if (!source) return <section className="project-detail project-detail-unavailable"><button type="button" onClick={() => setView("dashboard")}>← Volver a Proyectos</button><p>El proyecto ya no está disponible.</p></section>;
  const detail = projectDetail(source);
      const previewPiece = detail.assemblies.length > 0 ? source.project.pieces.find((piece) => documentFromProject({ ...selectPiecePage(source.project, piece), activePieceId: piece.id }).elements.length > 0) ?? source.project.pieces[0] : undefined;
      const previewProject = previewPiece ? { ...selectPiecePage(source.project, previewPiece), activePieceId: previewPiece.id } : source.project;
      const previewDocument = documentFromProject(previewProject);
      const preview = renderSvg(previewDocument, { zoom: 1, panMm: { x: 0, y: 0 } });
      const previewSvg = detail.assemblies.length > 0 && previewDocument.elements.length > 0 && preview.success ? preview.svg : undefined;
      const openPreview = () => previewPiece ? openProjectInDesign(source, previewPiece.id, previewProject.activePageId) : startNewSketchOnPage(source, source.project.activePageId);
  const cropPiecePreview = (pieceDocument: DocumentSnapshot, svg: string) => {
    if (pieceDocument.elements.length === 0) return svg;
    const bounds = boundsOfElements(pieceDocument.elements);
    const padding = Math.max(24, Math.min(48, Math.max(bounds.width, bounds.height) * 0.25));
    const viewBox = `${bounds.x - padding} ${bounds.y - padding} ${Math.max(bounds.width, 1) + padding * 2} ${Math.max(bounds.height, 1) + padding * 2}`;
    return svg.replace(/viewBox="[^"]+"/, `viewBox="${viewBox}"`);
  };
  const piecePreviews = new Map(source.project.pieces.map((piece) => {
    const pieceDocument = documentFromProject(selectPiecePage(source.project, piece));
    const renderedPiece = renderSvg(pieceDocument, { zoom: 1, panMm: { x: 0, y: 0 } });
    return [piece.id, {
      elementCount: pieceDocument.elements.length,
      svg: pieceDocument.elements.length > 0 && renderedPiece.success ? cropPiecePreview(pieceDocument, renderedPiece.svg) : undefined,
    }] as const;
  }));
  return <section className="project-detail">
    <aside className="project-detail-sidebar" aria-label="Navegación del proyecto"><div className="project-detail-sidebar-brand"><span aria-hidden="true">◇</span><strong>{detail.name}</strong><small>{detail.status}</small></div><nav><a href="#resumen">Resumen</a><a href="#piezas">Piezas <span>{detail.metrics.pieceCount}</span></a><a href="#paginas">Páginas <span>{detail.metrics.pageCount}</span></a><a href="#ensamblajes">Ensamblajes</a></nav><button type="button" className="project-detail-back" onClick={() => setView("dashboard")}>← Volver a Proyectos</button></aside>
    <div className="project-detail-content">
      <header className="project-detail-heading"><div><p className="dashboard-eyebrow">NODRA / PROYECTO</p>{renameDraft === undefined ? <div className="project-rename-heading"><h1>{detail.name}</h1><span className="project-status">{detail.status}</span><button type="button" className="project-detail-secondary" onClick={() => setRenameDraft(source.metadata.name)}>Renombrar</button></div> : <form className="project-rename-form" onSubmit={(event) => { event.preventDefault(); void renamePersistedProject(source); }}><label>Nombre del proyecto<input aria-label="Nuevo nombre del proyecto" autoFocus required value={renameDraft} onChange={(event) => setRenameDraft(event.target.value)} /></label><button type="button" className="project-detail-secondary" onClick={() => setRenameDraft(undefined)}>Cancelar</button><button type="submit">Guardar nombre</button></form>}<p className="project-detail-updated">Actualizado el {new Date(detail.updatedAt).toLocaleDateString("es-AR")}</p></div><button type="button" onClick={() => openNewPieceForm(source)}>+ Nueva pieza</button></header>
      <section id="resumen" className="project-overview" aria-labelledby="project-overview-title">
        <article className="project-preview-card"><div className="project-section-heading"><div><p className="dashboard-eyebrow">VISTA GENERAL</p><h2 id="project-overview-title">Vista previa del proyecto</h2></div><button type="button" className="project-detail-secondary" onClick={openPreview}>{previewPiece ? "Abrir en Modelo" : "Crear primer croquis"}</button></div><button type="button" className="project-preview" onClick={openPreview} aria-label={previewPiece ? "Abrir vista previa en Modelo" : "Crear primer croquis"}>{previewSvg ? <div className="project-preview-svg" dangerouslySetInnerHTML={{ __html: previewSvg }} /> : <span className="project-preview-empty"><strong>{detail.assemblies.length > 0 ? "Sin geometría para previsualizar" : "Sin ensamblaje para previsualizar"}</strong><small>{detail.assemblies.length > 0 ? "Abrí la pieza para comenzar a diseñar." : "Creá un ensamblaje para ver la vista del proyecto."}</small></span>}</button><footer>{detail.assemblies.length > 0 ? <><span>{previewDocument.page.width} × {previewDocument.page.height} {detail.units}</span><span>{previewDocument.elements.length} objetos en la vista</span></> : <span>Sin ensamblajes para previsualizar</span>}</footer></article>
        <article className="project-information-card"><p className="dashboard-eyebrow">INFORMACIÓN</p><h2>Información del proyecto</h2><dl><div><dt>Identificador</dt><dd>{detail.id}</dd></div><div><dt>Revisión</dt><dd>{detail.revision}</dd></div><div><dt>Unidades</dt><dd>{detail.units}</dd></div><div><dt>Materiales</dt><dd>{detail.materials.length ? detail.materials.join(", ") : "Sin definir"}</dd></div></dl></article>
      </section>
      <section className="project-metrics" aria-label="Métricas del proyecto">{[{ label: "Piezas", value: detail.metrics.pieceCount }, { label: "Páginas", value: detail.metrics.pageCount }, { label: "Croquis", value: detail.metrics.sketchCount }, { label: "Objetos", value: detail.metrics.elementCount }].map((metric) => <article key={metric.label}><strong>{metric.value}</strong><span>{metric.label}</span></article>)}</section>
      <section id="piezas" className="project-dashboard-section project-pieces-section" aria-labelledby="project-pieces-title">
        <div className="project-section-heading">
          <div><h2 id="project-pieces-title">Piezas del proyecto</h2></div>
          <button type="button" onClick={() => openNewPieceForm(source)}>+ Nueva pieza</button>
        </div>
        {detail.pieces.length === 0 && <div className="project-dashboard-empty"><strong>Todavía no hay piezas</strong><p className="muted">Todavía no hay piezas. Abrí Modelo para crear el primer croquis.</p><button type="button" onClick={() => startNewSketchOnPage(source, source.project.activePageId)}>Crear primer croquis</button></div>}
        <div className="project-piece-grid">{detail.pieces.map((piece) => {
          const piecePreview = piecePreviews.get(piece.id as PieceId);
          const stateClass = piece.state === "Lista" || piece.state === "Validada" ? "ready" : piece.state === "Subdefinida" ? "warning" : "active";
          return <article className="project-piece-card" key={piece.id}>
            <button type="button" className="project-piece-card-preview" onClick={() => openProjectInDesign(source, piece.id as PieceId)} aria-label={`Abrir ${piece.name}`}>
              {piecePreview?.svg ? <span className="project-piece-preview-svg" dangerouslySetInnerHTML={{ __html: piecePreview.svg }} /> : <span className="project-piece-preview-empty"><span aria-hidden="true">◇</span><small>Sin geometría</small></span>}
            </button>
            <div className="project-piece-card-body">
              <div className="project-piece-title"><div>{inlineRename(source, "piece", piece.id, piece.name)}<p>{piece.material || "Material sin definir"} · {piece.thicknessMm === undefined ? "Espesor sin definir" : `${piece.thicknessMm} mm`}</p></div></div>
              <div className="project-piece-meta"><span className={`project-piece-status ${stateClass}`}><i aria-hidden="true" />{piece.state}</span><span className="project-piece-summary">{piecePreview?.elementCount ?? 0} {(piecePreview?.elementCount ?? 0) === 1 ? "objeto" : "objetos"}</span></div>
              <div className="project-piece-actions"><button type="button" className="project-piece-primary" onClick={() => piece.sketches.length > 0 ? openProjectInDesign(source, piece.id as PieceId) : openEditPieceForm(source, piece.id as PieceId)}>{piece.sketches.length > 0 ? "Abrir croquis" : "Editar pieza"}</button><details className="project-piece-overflow"><summary aria-label={`Más acciones para ${piece.name}`} title="Más acciones">•••</summary><div className="project-piece-overflow-menu">{piece.sketches.map((sketch) => <button type="button" className="project-detail-secondary" key={`${sketch.pageId}:${sketch.sketchId}`} onClick={() => openProjectInDesign(source, piece.id as PieceId, sketch.pageId, sketch.sketchId as ElementId)}>Editar {sketch.label}</button>)}<button type="button" className="project-detail-danger" disabled={source.project.pieces.length <= 1} onClick={() => setDeletePieceId(piece.id as PieceId)}>Eliminar pieza</button></div></details></div>
            </div>
          </article>;
        })}</div>
      </section>
      <section id="paginas" className="project-dashboard-section" aria-labelledby="project-pages-title"><div className="project-section-heading"><div><p className="dashboard-eyebrow">DOCUMENTO</p><h2 id="project-pages-title">Layouts / Páginas</h2></div></div><ul className="project-page-list">{detail.pages.map((page) => <li key={page.id}><div>{inlineRename(source, "page", page.id, page.label)}<small>{page.sketchCount} croquis · {page.pieces.length} piezas</small></div>{source.project.pieces.length > 0 ? <button type="button" className="project-detail-secondary" onClick={() => openProjectInDesign(source, source.project.pieces.find((candidate) => piecePageId(source.project, candidate) === page.id)?.id ?? source.project.pieces[0]!.id, page.id)}>Abrir en Modelo</button> : <button type="button" onClick={() => startNewSketchOnPage(source, page.id)}>Crear primer croquis</button>}</li>)}</ul></section>
      <section id="ensamblajes" className="project-dashboard-section project-assemblies" aria-labelledby="project-assemblies-title"><div className="project-section-heading"><div><p className="dashboard-eyebrow">PRÓXIMA ETAPA</p><h2 id="project-assemblies-title">Ensamblajes</h2></div></div><div className="project-dashboard-empty"><span aria-hidden="true">◇</span><strong>Todavía no hay ensamblajes</strong><p className="muted">Esta sección queda preparada para una futura versión.</p></div></section>
    </div>
  </section>;
};
return <main className={`app-shell${persistenceReady ? "" : " hydrating"}`}>
...
<header className="topbar">
      ...
      <nav aria-label="Navegación principal"><button className={view === "dashboard" ? "active" : ""} onClick={() => guardedNavigation(() => { setView("dashboard"); void refreshDashboard(); })}>Proyectos</button><button className={mode === "design" && view === "editor" ? "active" : ""} onClick={() => guardedNavigation(() => { setView("editor"); setMode("design"); })}>Modelo</button><button className={mode === "prepare" ? "active" : ""} onClick={() => guardedNavigation(() => { setView("editor"); setMode("prepare"); })}>Preparar <small>Vista previa</small></button></nav>
      <div className="top-actions">{savePolicy.mode === "prompted-autosave" && <span className="autosave-indicator" role="img" aria-label="Autosave activo" />}<button type="button" className={savePolicy.mode === "prompted-autosave" ? "active" : ""} aria-pressed={savePolicy.mode === "prompted-autosave"} onClick={() => setSaveMode(savePolicy.mode === "prompted-autosave" ? "manual" : "prompted-autosave")}>Autosave: {savePolicy.mode === "prompted-autosave" ? "Activo" : "Inactivo"}</button>{sketchSession.status === "active" && <><span role="status">Editando croquis</span><button type="button" onClick={acceptSketchSession}>Aceptar</button><button type="button" onClick={requestCancelSketchSession}>Cancelar</button></>}{sketchSession.status === "confirming-cancel" && <span role="status">Confirmá la cancelación</span>}{desktopFileBridge() && <><button aria-label="Abrir proyecto" title="Abrir proyecto" onClick={() => guardedNavigation(() => void openDesktopProject())}>Abrir</button><button aria-label="Guardar proyecto" title="Guardar proyecto" onClick={() => void saveDesktopProject()}>Guardar</button><button aria-label="Guardar como" title="Guardar como" onClick={() => void saveDesktopProject(true)}>Guardar como</button></>}<button aria-label="Deshacer" onClick={() => { if (!isSketchSessionHistoryLocked(sketchSessionRef.current)) setEditorState(undo(editorRef.current)); }}>↶</button><button aria-label="Rehacer" onClick={() => { if (!isSketchSessionHistoryLocked(sketchSessionRef.current)) setEditorState(redo(editorRef.current)); }}>↷</button><span className="project-name">{projectDisplayName(activeProjectMetadata)}</span></div>
    </header>
    {view === "dashboard" ? <section className="dashboard-view"><div className="dashboard-hero"><div><p className="dashboard-eyebrow">NODRA / PROYECTOS</p><h1>¿Qué querés diseñar hoy?</h1><p className="muted">Tus proyectos guardados localmente, listos para continuar.</p></div><button type="button" className="dashboard-primary" onClick={() => guardedNavigation(() => setNewProjectDraft({ projectName: "" }))}>+ Nuevo proyecto</button></div><div className="dashboard-future" aria-label="Funciones futuras"><span>3D <small>Próximamente</small></span><span>Validar <small>Próximamente</small></span></div><div className="dashboard-grid">{dashboardProjects(dashboardSources).map((item) => <article className="project-card" key={item.id}><div className="project-card-preview" aria-hidden="true">◇</div><div className="project-card-body"><h2>{projectDisplayName(item)}</h2><div className="project-piece-list">{item.pieces.map((piece) => <p key={piece.id}>{pieceDisplayLabel(piece)}</p>)}</div><button type="button" onClick={() => { const source = dashboardSources.find((candidate) => candidate.metadata.id === item.id); if (source) openNewPieceForm(source); }}>Nueva pieza</button><small>{new Date(item.updatedAt).toLocaleDateString("es-AR")}</small><button type="button" onClick={() => guardedNavigation(() => void openPersistedProject(item))}>Ver proyecto</button><button type="button" onClick={() => setDeleteProjectId(item.id)}>Eliminar proyecto</button></div></article>)}{dashboardSources.length === 0 && <div className="dashboard-empty"><strong>Todavía no hay proyectos</strong><p className="muted">Creá tu primer proyecto para comenzar.</p><button type="button" className="dashboard-primary" aria-label="Crear mi primer proyecto" onClick={() => guardedNavigation(() => setNewProjectDraft({ projectName: "" }))}>+ Nuevo proyecto</button></div>}</div></section> : view === "project" ? renderProjectDetail() : !activeProjectMetadata.id ? mode === "prepare" ? <section className="prepare"><div><div className="prepare-icon">◇</div><h1>Preparar aún no está disponible</h1><p>Nodra ofrece actualmente solo un espacio de trabajo de Diseño sin conexión. No hay hardware conectado, controlado ni listo.</p><button type="button" onClick={() => { setMode("design"); setView("editor"); }}>Volver a Modelo</button></div></section> : <section className="prepare"><div><div className="prepare-icon">◇</div><h1>Empezá un proyecto</h1><p>Creá tu primer modelo para comenzar a trabajar.</p><button type="button" onClick={() => setNewProjectDraft({ projectName: "" })}>+ Nuevo proyecto</button></div></section> : project.pieces.length === 0 ? <section className="prepare empty-project-state"><div><div className="prepare-icon">◇</div><h1>Modelo vacío</h1><p>Este proyecto todavía no tiene piezas. Creá el primer croquis para empezar.</p><button type="button" onClick={() => startNewSketchOnPage({ metadata: activeProjectMetadata, project }, project.activePageId)}>Crear primer croquis</button></div></section> : mode === "prepare" ? <section className="prepare"><div><div className="prepare-icon">◇</div><h1>Preparar aún no está disponible</h1><p>Nodra ofrece actualmente solo un espacio de trabajo de Diseño sin conexión. No hay hardware conectado, controlado ni listo.</p><button onClick={() => setMode("design")}>Volver a Modelo</button></div></section> : <div className="workspace">
        ...
        {tool === "dimension" && <div className="dimension-mode-controls" role="group" aria-label="Modo de cota"><button type="button" className={dimensionMode === "auto" ? "active" : ""} onClick={() => setDimensionMode("auto")}>Lineal</button><button type="button" className={dimensionMode === "radius" ? "active" : ""} onClick={() => setDimensionMode("radius")}>Radio</button><button type="button" className={dimensionMode === "diameter" ? "active" : ""} onClick={() => setDimensionMode("diameter")}>Diámetro</button></div>}
         <div className="page-selector"><label>Página<select aria-label="Página activa" value={project.activePageId} onChange={(event) => switchPage(event.target.value)}>{project.pages.map((page, index) => <option key={page.id} value={page.id}>{page.name ?? `Página ${index + 1}`} · {page.page.width} × {page.page.height} mm</option>)}</select></label></div>
      </section>
         <aside className="project-tree" aria-label="Árbol del proyecto"><div className="project-tree-heading"><span className="project-tree-title">{treeIcon("project")}<strong>Árbol del proyecto</strong></span><button type="button" className="project-tree-add" aria-label="Nueva página" title="Nueva página" onClick={() => void createPersistedPage()}>+</button></div><ul>{projectTree({ metadata: activeProjectMetadata, project }).pages.map((page) => { const collapsed = collapsedPages.has(page.id); return <li key={page.id}>{inlineRename({ metadata: activeProjectMetadata, project }, "page", page.id, page.label, () => guardedNavigation(() => switchPage(page.id)), project.activePageId === page.id ? "active" : "", treeIcon("page"))}{page.pieces.length > 0 && <button type="button" className="project-tree-disclosure" aria-label={`${collapsed ? "Expandir" : "Colapsar"} ${page.label}`} aria-expanded={!collapsed} aria-controls={`project-page-${page.id}-children`} onPointerDown={(event) => event.stopPropagation()} onClick={(event) => { event.stopPropagation(); setCollapsedPages((current) => { const next = new Set(current); if (next.has(page.id)) next.delete(page.id); else next.add(page.id); return next; }); }}>{collapsed ? "▸" : "▾"}</button>}<button type="button" className="project-tree-new-sketch" onClick={() => startNewSketchOnPage({ metadata: activeProjectMetadata, project }, page.id)}>Nuevo croquis</button> {page.pieces.length > 0 && !collapsed && <ul id={`project-page-${page.id}-children`}>{page.pieces.map((piece) => <li key={piece.id}>{inlineRename({ metadata: activeProjectMetadata, project }, "piece", piece.id, piece.name, () => activatePiece(piece.id as PieceId), activePieceId === piece.id ? "active" : "", treeIcon("piece"))}{piece.sketches.length > 0 && <ul>{piece.sketches.map((sketch: { readonly pageId: string; readonly sketchId: string; readonly label: string }) => <li key={`${sketch.pageId}:${sketch.sketchId}`}><button type="button" onClick={() => openProjectInDesign({ metadata: activeProjectMetadata, project }, piece.id as PieceId, sketch.pageId, sketch.sketchId as ElementId)}>{treeIcon("sketch")}{sketch.label}</button></li>)}</ul>}</li>)}</ul>}</li>; })}</ul></aside><section className="workspace-tools"><div className="tool-column" role="toolbar" aria-label="Herramientas de diseño">{(["select", "forma", "pen", "spline", "text", "rectangle", "circle", "line", "arc", "cut", "dimension", "pan"] as const).map((item) => <ToolButton key={item} label={toolCursorLabels[item]} icon={item} active={tool === item} onClick={() => { setTransformMode("resize"); setPenDraftPoint(undefined); creationDraftRef.current = undefined; setCreationDraft(undefined); if (item === "forma" && selectedElements.some((element) => element.type === "text")) setEditorState(clearSelection(editorRef.current)); if (item !== "forma") setEditModeElementIds([]); setTool(item); }} />)}{selectedElements.length > 0 && <>{rotateSelectionButton()}{mirrorButton("horizontal")}{mirrorButton("vertical")}</>}<div className="zoom-controls"><button aria-label="Alejar" onClick={() => setZoom(zoom - 0.5)}>−</button><span className="zoom-label">{Math.round(zoom * 100 / 3)}%</span><button aria-label="Acercar" onClick={() => setZoom(zoom + 0.5)}>+</button></div></div></section>
     ...
     <footer className="statusbar">{view === "editor" && mode === "design" && activeProjectMetadata.id && project.pieces.length > 0 && <div className="piece-context" aria-label="Contexto de pieza activa"><div className="piece-tabs" role="tablist" aria-label="Piezas">{project.pieces.filter((piece) => piecePageId(project, piece) === project.activePageId).map((piece) => <button type="button" role="tab" aria-selected={piece.id === activePiece?.id} className={piece.id === activePiece?.id ? "active" : ""} key={piece.id} onClick={() => activatePiece(piece.id)}>{piece.name}</button>)}<button type="button" onClick={() => guardedNavigation(() => { setPieceFormProjectId(activeProjectMetadata.id); setPieceName(nextPieceName(project)); setPieceMaterial(""); setPieceThickness(""); })}>+ Nueva pieza</button></div></div>}{view === "editor" && mode === "design" && activeProjectMetadata.id && project.pieces.length > 0 && paletteControls()}</footer>
  </main>;
}

const toolDescriptions: Record<string, string> = { Seleccion: "Seleccione, coloque o transforme objetos.", Forma: "Editar la forma mediante sus nodos.", Pluma: "Cree un trazado Bézier con clics y arrastre para editar sus controles.", Spline: "Cree curvas con nodos y handles. Haga clic en el primer nodo para cerrarla; arrastre nodos y controles para editarla.", Texto: "Escriba texto editable sobre la hoja.", Rectángulo: "Dibuje formas rectangulares.", Círculo: "Fije el centro, defina el radio y haga clic para crear un círculo.", Línea: "Cree líneas por segmentos con clics; haga clic en el primer nodo para cerrar.", Arco: "Cree un arco con tres clics: inicio, final y punto de paso.", Desplazar: "Desplace el espacio de trabajo.", Cota: "Mida y cree cotas asociativas entre nodos y líneas.", Radio: "Cree una cota radial explícita desde el centro hasta el borde de un círculo." };
function ToolIcon({ icon }: { icon: Tool }) {
  const shape = icon === "select" ? <><path d="M5 3 18 12l-6 1.5L10 20 5 3Z" /><path d="m13 15 4 4" /></> : icon === "text" ? <><path d="M5 5h14M12 5v14M8 19h8" /><path d="M7 5v2M17 5v2" /></> : icon === "forma" ? <><path d="M6 6h12v12H6z" /><path d="M6 12h12M12 6v12" /><circle cx="6" cy="6" r="1.5" /><circle cx="18" cy="6" r="1.5" /><circle cx="6" cy="18" r="1.5" /><circle cx="18" cy="18" r="1.5" /></> : icon === "pen" ? <><path d="m4 19 5-5 7-7 4 4-7 7-5 1Z" /><path d="m14 8 2-2 4 4-2 2" /><path d="M4 19h4" /><circle cx="9" cy="14" r="1.25" /></> : icon === "spline" ? <><path d="M4 18c3-8 6-10 9-6s4 4 7-3" /><path d="M4 18h5M13 12l-2-5M13 12l4 2" /><circle cx="4" cy="18" r="1.5" /><circle cx="13" cy="12" r="1.5" /><circle cx="20" cy="9" r="1.5" /></> : icon === "rectangle" ? <><rect x="5" y="5" width="14" height="14" /><path d="M5 8V5h3M16 5h3v3M19 16v3h-3M8 19H5v-3" /></> : icon === "circle" ? <><circle cx="12" cy="12" r="7" /><path d="M12 3v3M12 18v3M3 12h3M18 12h3" /></> : icon === "radius" ? <><circle cx="12" cy="12" r="7" /><path d="M12 12 18 7M12 9v3h3" /><circle cx="12" cy="12" r="1" /></> : icon === "line" ? <><path d="M5 19 19 5" /><circle cx="5" cy="19" r="1.5" /><circle cx="19" cy="5" r="1.5" /></> : icon === "arc" ? <><path d="M4 18a8 8 0 0 1 16 0" /><path d="M4 18h3M17 18h3M12 10v3M9 13h6" /><circle cx="12" cy="13" r="1" /></> : icon === "cut" ? <><circle cx="6" cy="7" r="2" /><circle cx="6" cy="17" r="2" /><path d="m8 8 11 8M8 16 12 13M15 10l4-3" /><path d="m12 11 1 1" /></> : icon === "dimension" ? <><path d="M5 4v16M19 4v16M5 12h14" /><path d="M8 9 5 12l3 3M16 9l3 3-3 3" /><path d="M8 4h8M8 20h8" /></> : <><path d="M12 4v16M4 12h16" /><path d="m9 7 3-3 3 3M9 17l3 3 3-3M7 9l-3 3 3 3M17 9l3 3-3 3" /><circle cx="12" cy="12" r="1" /></>;
  return <svg className="tool-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">{shape}</svg>;
}
