...
import { circleGeometry, creationGuides, cursorNodeGuide, directionalGuide, lineAngleDegrees, nodeAlignmentGuide, type CreationGuide } from "./interaction.js";
  ...
  const marqueeStyle = marquee ? (() => {
   ...
   const pendingDimensionOverlay = tool === "dimension" && dimensionDraft && cursorPoint ? (() => {
    ...
    return <svg className="dimension-pending-overlay" aria-hidden="true"><line x1={start.x} y1={start.y} x2={end.x} y2={end.y} /><circle cx={start.x} cy={start.y} r="7" /><circle cx={end.x} cy={end.y} r="7" /><text x={start.x + 10} y={start.y - 10}>Primer nodo</text><text x={end.x + 12} y={end.y - 12}>{label}</text></svg>;
     ...
     const cursorNodeGuideOverlay = tool === "line" && !creationDraft && documentCursorPoint ? (() => { const guide = cursorNodeGuide(document, documentCursorPoint, zoom, 5); return guide ? <svg className="node-guide-overlay" viewBox={`0 0 ${document.page.width} ${document.page.height}`} style={{ left: pageStyle.left + 1, top: pageStyle.top + 1, width: document.page.width * zoom, height: document.page.height * zoom, right: "auto", bottom: "auto" }} aria-label="Guía de nodo"><line className="creation-guide creation-guide-node-alignment" x1={guide.source.x} y1={guide.source.y} x2={guide.target.x} y2={guide.target.y} /></svg> : undefined; })() : undefined;
