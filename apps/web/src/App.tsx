...
const mark = globalThis.document.createElementNS("http://www.w3.org/2000/svg", "rect");
  ...
  useEffect(() => {
    ...
    const selectedCircularRelation = selectedElements.find((element): element is Extract<Element, { type: "circle" | "arc" }> => element.type === "circle" || element.type === "arc");
    const selectedPositionalNodes = selectedElements.flatMap((element) => selectedFormaNodeKeys.flatMap((key) => {
      const match = key.match(new RegExp(`^${element.id}:p:(\\d+)$`));
      const nodeIndex = match ? Number(match[1]) : -1;
      const node = nodeIndex >= 0 ? realGeometryNodes(element)[nodeIndex] : undefined;
      const address = node && nodeIndex >= 0 ? connectableNodeAddress(element, nodeIndex) : undefined;
      return node && address ? [{ element, nodeIndex, node, address }] : [];
    }));
    const selectedCircularSource = selectedCircularRelation ? selectedPositionalNodes.find(({ element, address }) => element.id === selectedCircularRelation.id && address.kind === "named" && ["center", "start", "end", "n", "e", "s", "w"].includes(address.name)) : undefined;
    const selectedPositionalTarget = selectedPositionalNodes.find(({ element }) => element.id !== selectedCircularRelation?.id);
    ...
    const globalResiduals = useMemo(() => new Map(constraintResidualsForDocument(document).map((residual) => [residual.constraintId, residual])), [document]);
        ...
        const confirmConstraintDraft = () => { if (!constraintDraft) return; setEditorState(commitGesture(editorRef.current)); setConstraintDraft(undefined); };
         ...
         const addNativePositionRelation = () => {
           if (!selectedCircularSource || !selectedPositionalTarget) return;
           const next = dispatch(editorRef.current, addPositionalConnection({ elementId: selectedCircularSource.element.id, node: selectedCircularSource.address }, { elementId: selectedPositionalTarget.element.id, node: selectedPositionalTarget.address }));
