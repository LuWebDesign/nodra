...
import { addDocumentConstraint, deleteDocumentConstraint, addSketchConstraint, addToSelection, appendSketchEdge, appendSplineNode, beginGesture, cancelGesture, clearSelection, closePath, cutLineAtPoint, cutPathSegment, cutSketchEdge, closeSplineElement, commitGesture, convertTextToGlyphs, createElement, createPathCubicNode, createPathNode, createSketchLine, deleteContourNodes, deleteElement, deleteElementNodes, deletePathNodes, deleteSketchConstraint, dispatch, duplicateElements, flipElements, insertFormaNode, invalidDimensionIdsForShapeOperation, moveElements, movePathHandle, movePathNode, openPath, updateSplineNode, previewGesture, previewGestureFromBase, redo, resizeElement, resizeElementToDimensions, resizeElements, resizeElementsToDimensions, rotateElement, updateDimensionValue, updateDocumentConstraint, setDimensionDriving, rotateElementsAroundCenter, select, selectForPointerDown, setPathJoin, shapeOperation, splitPathSegment, undo, updateContourNode, updateElement, updateElementNode, updateElementStyles, updatePage, updateSketchConstraint, updateSplineHandle, type EditorCommand, type FlipAxis, type ShapeOperation } from "@nodra/editor-core";
import { constraintResidualsForDocument, solveConstraintComponents } from "@nodra/constraints";
      ...
      if (tool === "forma" && rawFormaNodeHit) {
        ...
        if ((tool === "dimension" || tool === "radius")) {
           ...
           if (dimensionDraft?.phase === "placement") {
            ...
            const created = dispatch(editorRef.current, createElement(dimension));
            if (created === editorRef.current) return;
            const next = dimension.kind === "radius" || dimension.kind === "diameter" || shouldCreateSketchDrivingConstraint(dimension) ? dispatch(created, setDimensionDriving(dimension.id, true)) : created;
  ...
  useEffect(() => {
    ...
    const globalResiduals = useMemo(() => new Map(constraintResidualsForDocument(document).map((residual) => [residual.constraintId, residual])), [document]);
        ...
        const confirmConstraintDraft = () => { if (!constraintDraft) return; setEditorState(commitGesture(editorRef.current)); setConstraintDraft(undefined); };
         ...
         const saveDocumentConstraintValue = (constraint: SketchConstraint) => { const value = Number(constraintValueEdits[constraint.id] ?? constraint.value); if (!Number.isFinite(value) || value <= 0) return; const command: EditorCommand = { name: `document-constraint-value:${constraint.id}`, apply: (current) => { const updated = updateDocumentConstraint({ ...constraint, value }).apply(current); if (!updated.success) return updated; const solved = solveConstraintComponents(updated.document); const residual = constraintResidualsForDocument(solved.document).find((candidate) => candidate.constraintId === JSON.stringify(["document", null, constraint.id])); return !residual?.supported || !residual.satisfied || solved.states.some((state) => state.state === "conflict") ? { success: false, error: "La relación global entra en conflicto" } : { ...updated, document: solved.document }; } }; const next = dispatch(editorRef.current, command); if (next !== editorRef.current) { setEditorState(next); setConstraintValueEdits((current) => { const nextValues = { ...current }; delete nextValues[constraint.id]; return nextValues; }); } };
