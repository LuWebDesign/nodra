    ...
    {mode === "prepare" ? <section className="prepare"><div><div className="prepare-icon">◇</div><h1>Preparar aún no está disponible</h1><p>Nodra ofrece actualmente solo un espacio de trabajo de Diseño sin conexión. No hay hardware conectado, controlado ni listo.</p><button onClick={() => setMode("design")}>Volver a Diseño</button></div></section> : <div className="workspace">
      ...
      <section className="canvas-area">
        <header className="canvas-header"><span>DISEÑO / SIN TÍTULO</span><span>{document.elements.length} objetos · {document.page.width} × {document.page.height} mm</span><div className="zoom-controls"><button aria-label="Alejar" onClick={() => setZoom(zoom - 0.5)}>−</button><span className="zoom-label">{Math.round(zoom * 100 / 3)}%</span><button aria-label="Acercar" onClick={() => setZoom(zoom + 0.5)}>+</button></div></header>
           ...
           {contourNodes.length > 0 && <div className="contour-node-overlay" role="group" aria-label="Nodos de forma">{contourNodes.map((node) => { const screen = pagePointToCanvas(node.point, zoom, panMm); return <button key={`${node.elementId}:${node.ringIndex}:${node.pointIndex}`} type="button" className="contour-node" data-contour-node={`${node.elementId}:${node.ringIndex}:${node.pointIndex}`} aria-label={`Nodo del contorno, anillo ${node.ringIndex + 1}, punto ${node.pointIndex + 1}`} style={{ left: screen.x, top: screen.y }} onPointerDown={(event) => { event.preventDefault(); event.stopPropagation(); event.currentTarget.setPointerCapture(event.pointerId); setEditorState(beginGesture(editorRef.current)); interaction.current = { pointerId: event.pointerId, lastX: event.clientX, lastY: event.clientY, kind: "contour-node", contourNode: node, startClient: { x: event.clientX, y: event.clientY }, dragged: false }; }} onPointerUp={(event) => finishPointer(event, false)} onPointerCancel={(event) => finishPointer(event, true)} />; })}</div>}
