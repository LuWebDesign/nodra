...
function renderElement(element: Element, viewport: Viewport, document: DocumentSnapshot, sketchConstraintStates: ReadonlyMap<string, ConstraintState>, mode: RenderMode): string {
  ...
  if (element.type === "dimension") {
    ...
    const value = `${geometry.kind === "diameter" ? "Ø" : geometry.kind === "radius" ? "R" : ""}${geometry.value.toFixed(element.precision)} ${element.units}`;
  ...
  if (element.type === "circle") {
    const center = screen(element.center);
    const radius = element.radius * viewport.zoom;
    return `<circle data-element-id="${escapeAttribute(element.id)}" cx="${number(center.x)}" cy="${number(center.y)}" r="${number(radius)}" ${visualAttributes(element)} />`;
  }
...
function renderDimension(element: Extract<Element, { type: "dimension" }>, viewport: Viewport, elements: readonly Element[]): string {
  ...
  if (geometry.kind === "radius" || geometry.kind === "diameter") {
    if (!geometry.leaderStart || !geometry.leaderEnd) return "";
    const leaderStart = screen(geometry.leaderStart); const leaderEnd = screen(geometry.leaderEnd);
    return `<g data-element-id="${escapeAttribute(element.id)}" data-dimension="${geometry.kind}" stroke="${stroke}" stroke-width="${number(element.style.strokeWidth)}" fill="none" vector-effect="non-scaling-stroke"><defs><marker id="${markerId}" viewBox="0 0 10 10" refX="5" refY="5" markerWidth="7" markerHeight="7" orient="auto"><path d="M0 0 10 5 0 10 2.5 5Z" fill="${stroke}" stroke="none" /></marker></defs><line x1="${number(leaderStart.x)}" y1="${number(leaderStart.y)}" x2="${number(leaderEnd.x)}" y2="${number(leaderEnd.y)}" marker-end="url(#${markerId})" /><text x="${number(text.x)}" y="${number(text.y - 8)}" text-anchor="middle" dominant-baseline="middle" fill="${stroke}" stroke="#ffffff" stroke-width="3" paint-order="stroke" font-size="18" font-weight="600">${escapeAttribute(value)}</text></g>`;
  }
