...
export function splitCurveAtParameters(c: Curve2D, parameters: readonly number[], parameterEpsilon = PARAMETER_EPSILON): readonly CurveFragment[] { validate(c); if (!parameters.every(Number.isFinite) || parameters.some((t) => t < 0 || t > 1)) throw new Error("parameters must be finite and within [0, 1]"); if (!Number.isFinite(parameterEpsilon) || parameterEpsilon < 0 || parameterEpsilon >= 0.5) throw new Error("parameterEpsilon must be finite and within [0, 0.5)"); const cuts = parameters.slice().filter((t) => t > parameterEpsilon && t < 1 - parameterEpsilon).sort((a, b) => a - b).filter((t, i, all) => i === 0 || t - all[i - 1]! > parameterEpsilon); const bounds = [0, ...cuts, 1]; const out: CurveFragment[] = []; for (let i = 0; i + 1 < bounds.length; i++) { const t0 = bounds[i]!, t1 = bounds[i + 1]!; let piece: Curve2D; if (c.type === "line") piece = { type: "line", start: pointAt(c, t0), end: pointAt(c, t1) }; else if (c.type === "cubicBezier") { const right = splitCubic(c, t0)[1]; piece = t0 === 0 && t1 === 1 ? c : splitCubic(right, (t1 - t0) / (1 - t0))[0]; } else if (c.type === "circle") piece = { type: "arc", center: c.center, radius: c.radius, startAngle: TAU * t0, endAngle: TAU * t1, direction: "clockwise", ...(t0 === 0 && t1 === 1 ? { fullTurn: true } : {}) }; else piece = { type: "arc", center: c.center, radius: c.radius, startAngle: arcAngle(c, t0), endAngle: arcAngle(c, t1), direction: c.direction, ...(t0 === 0 && t1 === 1 && c.fullTurn ? { fullTurn: true } : {}) }; out.push({ curve: piece, sourceInterval: { t0, t1 } }); } return out; }
