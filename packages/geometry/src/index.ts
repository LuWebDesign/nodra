...
export function lineSegmentIntersection(firstStart: PointMm, firstEnd: PointMm, secondStart: PointMm, secondEnd: PointMm, epsilon = 1e-9): SegmentIntersection | undefined {
  ...
  const firstDelta = { x: firstEnd.x - firstStart.x, y: firstEnd.y - firstStart.y }; const secondDelta = { x: secondEnd.x - secondStart.x, y: secondEnd.y - secondStart.y };
  if (Math.abs(firstDelta.x * secondDelta.y - firstDelta.y * secondDelta.x) <= epsilon) return undefined;
  const result = intersectCurves({ type: "line", start: firstStart, end: firstEnd }, { type: "line", start: secondStart, end: secondEnd }, { geometryEpsilon: epsilon, parameterEpsilon: epsilon });
  const intersection = result.kind === "points" ? result.points[0] : undefined;
  return intersection ? { point: intersection.point, firstT: intersection.firstParameter, secondT: intersection.secondParameter } : undefined;
}
...
export function cubicBezierLineIntersections(curve: CubicBezier, lineStart: PointMm, lineEnd: PointMm, epsilon = 1e-9): readonly CubicBezierLineIntersection[] {
  ...
  if ([curve.p1, curve.p2, curve.p3].every((point) => point.x === curve.p0.x && point.y === curve.p0.y)) return [];
  try {
    const result = intersectCurves({ type: "cubicBezier", ...curve }, { type: "line", start: lineStart, end: lineEnd }, { geometryEpsilon: epsilon, parameterEpsilon: 1e-12 });
    return result.kind === "points" ? result.points.map(({ point, firstParameter, secondParameter }) => ({ point, curveT: firstParameter, lineT: secondParameter })) : [];
  } catch (error) {
    if (error instanceof Error && error.message.includes("numeric range")) throw new Error("curve and segment exceed the numeric range");
    throw error;
  }
}
