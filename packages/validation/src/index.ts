...
const geometryRole = z.enum(["normal", "construction"]).default("normal");
...
const circle = z.object({ id: nonEmptyId, layerId: nonEmptyId, ...pieceOwnership, role: geometryRole, type: z.literal("circle"), center: point, radius: finite.gt(0), style, operation: operation.optional(), circleConstraints: z.array(circleConstraint).optional() }).strict().superRefine((value, ctx) => {
...
const arc = z.object({ id: nonEmptyId, layerId: nonEmptyId, ...pieceOwnership, role: geometryRole, type: z.literal("arc"), center: point, radius: finite.gt(0), startAngle: finite.min(0).lt(2 * Math.PI), endAngle: finite.min(0).lt(2 * Math.PI), direction: z.enum(["clockwise", "counterclockwise"]), style, operation: operation.optional() }).strict().superRefine((value, ctx) => {
...
const connectableAddress = z.union([
  z.object({ kind: z.literal("named"), name: z.enum(["nw", "n", "ne", "e", "se", "s", "sw", "w", "center", "start", "end"]) }).strict(),
...
const sketchEdge = z.object({ id: nonEmptyId, startNodeId: nonEmptyId, endNodeId: nonEmptyId, role: geometryRole }).strict();
...
export const splineElementSchema = z.object({ id: nonEmptyId, layerId: nonEmptyId, ...pieceOwnership, role: geometryRole, type: z.literal("spline"), nodes: z.array(splineNodeSchema).min(2), closed: z.boolean(), style, operation: operation.optional() }).strict().superRefine((value, ctx) => {
...
const validateConnections = (elements: readonly z.infer<typeof elementSchema>[], connections: readonly z.infer<typeof explicitConnection>[], ctx: z.RefinementCtx, path: (string | number)[] = [], positional = false) => {
  ...
  connections.forEach((connection, index) => {
    ...
    refs.forEach((reference, referenceIndex) => {
      ...
      const validNamed = address.kind === "named" && (element?.type === "rectangle" || (element?.type === "circle" || element?.type === "ellipse") && ["center", "n", "e", "s", "w"].includes(address.name) || element?.type === "arc" && ["center", "start", "end"].includes(address.name));
  ...
  for (const [index, element] of value.elements.entries()) {
    ...
    if (element.type === "dimension") for (const [referenceIndex, reference] of element.references.entries()) {
      ...
      if (!elementIds.has(reference.elementId)) ctx.addIssue({ code: "custom", message: "Dimension references an unknown element", path: ["elements", index, "references", referenceIndex, "elementId"] });
        ...
        const nodeCount = target?.type === "line" ? 3 : target?.type === "sketch" ? target.nodes.length : target?.type === "rectangle" ? 9 : target?.type === "circle" || target?.type === "ellipse" || target?.type === "text" ? 5 : target?.type === "arc" ? 3 : target?.type === "contour" ? target.contours.reduce((count, contour) => count + Math.max(0, contour.points.length - 1) * 2, 0) : target?.type === "path" ? target.nodes.length + target.segments.filter((segment) => segment.type === "cubicBezier").length * 2 : target?.type === "spline" ? target.nodes.reduce((count, node) => count + 1 + Number(node.inHandle !== undefined) + Number(node.outHandle !== undefined), 0) : target?.type === "glyph" ? target.contours.reduce((count, contour) => count + contour.nodes.length + contour.segments.filter((segment) => segment.type === "cubicBezier").length * 2, 0) : undefined;
              ...
              const stableNodeIdsByIndex: readonly (string | undefined)[] = target?.type === "line" ? ["start", "center", "end"]
                : target?.type === "rectangle" ? ["nw", "ne", "se", "sw", "center", "n", "e", "s", "w"]
                  : target?.type === "circle" || target?.type === "ellipse" ? ["center", "n", "e", "s", "w"]
                    : target?.type === "arc" ? ["center", "start", "end"]
    ...
    if (element.type === "dimension" && (element.kind === "radius" || element.kind === "diameter")) {
      ...
      const nodeIds = target?.type === "circle" ? new Set(["center", "n", "e", "s", "w"]) : target?.type === "arc" ? new Set(["center", "start", "end"]) : undefined;
            if ((target?.type !== "circle" && target?.type !== "arc") || first.kind !== "node" || second.kind !== "node" || first.nodeId === undefined || second.nodeId === undefined || !nodeIds?.has(first.nodeId) || !nodeIds.has(second.nodeId) || first.nodeId === second.nodeId || first.nodeId !== "center" && second.nodeId !== "center") ctx.addIssue({ code: "custom", message: "Radial dimensions require distinct center and rim nodes on one circle or arc; arc dimensions may remain driving after a circle Trim", path: ["elements", index, "references"] });
    }
...
export const projectSchema = z.object({ schemaVersion: z.literal(CURRENT_SCHEMA_VERSION), id: nonEmptyId, revision: finite.int().nonnegative(), origin: z.literal("top-left"), units: z.literal("mm"), activePieceId: nonEmptyId.optional(), capabilities: z.object({ spline: z.literal(1).optional() }).strict().optional(), preferences: projectPreferencesSchema, pieces: z.array(pieceSchema), pages: z.array(pageSchema).min(1), activePageId: nonEmptyId }).strict().superRefine((value, ctx) => {
...
const legacyStableNodeIndex = (target: Record<string, unknown>, nodeId: string): number | undefined => {
  const named = target.type === "line" ? ["start", "center", "end"] : target.type === "rectangle" ? ["nw", "ne", "se", "sw", "center", "n", "e", "s", "w"] : target.type === "circle" || target.type === "ellipse" ? ["center", "n", "e", "s", "w"] : target.type === "arc" ? ["center", "start", "end"] : undefined;
...
const TAU = Math.PI * 2;
const arcPoints = (arc: Extract<Element, { type: "arc" }>): readonly PointMm[] => {
  const direction = arc.direction === "clockwise" ? 1 : -1;
  const sweep = ((arc.endAngle - arc.startAngle) * direction + TAU) % TAU;
  const angles = [arc.startAngle, arc.endAngle, 0, Math.PI / 2, Math.PI, Math.PI * 1.5].filter((angle, index) => index < 2 || (() => {
    const traveled = ((angle - arc.startAngle) * direction + TAU) % TAU;
    return traveled <= sweep + 1e-12;
  })());
  return angles.map((angle) => ({ x: arc.center.x + arc.radius * Math.cos(angle), y: arc.center.y + arc.radius * Math.sin(angle) }));
};
const elementPoints = (element: Element): readonly PointMm[] => {
  switch (element.type) {
    ...
    case "arc": return arcPoints(element);
