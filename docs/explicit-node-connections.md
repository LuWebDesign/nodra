...
Inspector width and height edits honor a connected side on the selected rectangle, native circle, or legacy ellipse. A left/top connection keeps that coordinate fixed, a right/bottom connection keeps the opposite coordinate fixed, and the other side moves. With no relevant connection, the existing centered resize behavior remains. Aspect lock applies the proportional dimension before applying the same anchor policy.
